...
with the Winix API (`WinixAuth`), managing a user account (`WinixAccount`), and interacting with Winix devices
(`WinixClient`).
...
the account and to access the Cognito identity id needed for device control.
...
// Get the Cognito identity id (required to build a WinixClient)
const identityId = account.getIdentityId();
```

### Interacting with a Device

`WinixClient` is an instance-based client. Construct it with the `identityId` resolved during account login. It
includes built-in rate-limit handling with a 60s cooldown after a `RateLimitError` is thrown.

```typescript
import { Airflow, AirQuality, Mode, Plasmawave, Power, WinixClient } from 'winix-api';
...
```typescript
const power: Power = await client.getPower(deviceId);
console.log('off?:', power === Power.Off);

// Set power on
await client.setPower(deviceId, Power.On);
```

#### Get and set the Mode

```typescript
const mode: Mode = await client.getMode(deviceId);
console.log('manual?:', mode === Mode.Manual);

// Set to auto
await client.setMode(deviceId, Mode.Auto);
```

#### Get and set the Airflow speed

```typescript
const airflow: Airflow = await client.getAirflow(deviceId);
console.log('turbo?:', airflow === Airflow.Turbo);

// Set to low
await client.setAirflow(deviceId, Airflow.Low);
```

#### Get the Air Quality

```typescript
const airQuality: AirQuality = await client.getAirQuality(deviceId);
...
```typescript
const plasma: Plasmawave = await client.getPlasmawave(deviceId);
console.log('plasmawave on?:', plasma === Plasmawave.On);

// Set to off
await client.setPlasmawave(deviceId, Plasmawave.Off);
```

#### Get the Ambient Light

```typescript
const ambientLight: number = await client.getAmbientLight(deviceId);
